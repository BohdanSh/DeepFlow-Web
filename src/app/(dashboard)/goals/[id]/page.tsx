...
export default function GoalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!goal) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center transition-colors">
        <div className="text-gray-500 dark:text-gray-400">Goal not found</div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white space-y-6 transition-colors">
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            {goal.category && (
              <span className="inline-block mt-2 text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full capitalize">
                {goal.category}
              </span>
            )}
            {goal.description && (
              <p className="mt-3 text-gray-500 dark:text-gray-400">{goal.description}</p>
            )}
            {goal.target_date && (
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between text-sm mb-2">
              ...
              <span className="font-medium text-gray-900 dark:text-white">
                <span className="text-green-600 dark:text-green-400">{completedTasks}</span>/{totalTasks} tasks ({progressPercent}%)
              </span>
            </div>
      ...
      {suggestedTasks.length > 0 && (
        <div className="bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/30 rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-purple-700 dark:text-purple-300">✨ AI Suggested Tasks</h3>
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
          Tasks
          {tasks.length > 0 && (
            <span className="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400">({tasks.length})</span>
          )}
        </h2>
        ...
        {tasks.length > 0 ? (
          <div className="space-y-2">
            {tasks.map((task) => (
              <div
                key={task.id}
                className={`flex items-start gap-3 bg-white dark:bg-gray-800 rounded-lg p-4 border-l-4 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-colors group shadow-sm ${
                  task.priority === 'high' ? 'border-l-red-500 bg-red-50 dark:bg-red-500/5' : 
                  task.priority === 'medium' ? 'border-l-yellow-500 bg-yellow-50 dark:bg-yellow-500/5' : 
                  'border-l-gray-400 dark:border-l-gray-500 bg-gray-50 dark:bg-gray-500/5'
                }`}
              >
                <button
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-purple-100 dark:bg-purple-600/20 text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-500/30 rounded-lg hover:bg-purple-200 dark:hover:bg-purple-600/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
